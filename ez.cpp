    ...
    for (size_t i = 0; i < src.size();)
    {
        ...
        if (isalpha(src[i]))
        {
            ...
            if (id == "when")
                ...
                tokens.push_back({INPUT, id, line});
            else if (id == "yes")
                tokens.push_back({BOOL, "1", line});
            else if (id == "no")
                tokens.push_back({BOOL, "0", line});
            else if (id == "escape")
                tokens.push_back({BREAK, id, line});
            else if (id == "skip")
                tokens.push_back({CONTINUE, id, line});
            else if (id == "and")
                tokens.push_back({AND, id, line});
            else if (id == "or")
                tokens.push_back({OR, id, line});
            else if (id == "not")
                tokens.push_back({NOT, id, line});
            else if (id == "get")
    ...
    void statement()
    {
        if (cur().type == PRINT)
        {
            ...
            do
            {
                ...
                if (!first)
                {
                    cout << " "; // space separator between arguments
                }
                cout << v.toString();
                first = false;

                // If there's a comma, continue to next argument
                if (cur().type == COMMA)
                {
                    next(); // consume comma
                }
            ...
            cout << endl; // final newline after all arguments
            return;
        }
