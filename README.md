...
A comprehensive mathematics library for the EZ programming language, providing trigonometric functions, vector operations, statistics, and more—all implemented from scratch using mathematical algorithms.

[![Version](https://img.shields.io/badge/version-1.0.0-blue.svg)](https://github.com/imabd645/EZmath)
...
// Basic trigonometry
angle = 45
out "cos(45°) =", cosd(angle)
out "sin(45°) =", sind(angle)
...
```ez
radians = deg_to_rad(180)  // π radians
```
...
**Implementation:** Uses the identity sin(x) = cos(π/2 - x)
...
// Calculate all trig functions for 30°
angle = 30
...
⚠️ **Note:** Negative inputs will cause an error.
...
⚠️ **Note:** Only positive inputs are valid.
...
task vector_field(x, y) {
    ...
    out "At (", x, ",", y, ") → (", normalized.x, ",", normalized.y, ")"
}
...
task analyze_data(dataset) {
    ...
    out count, "values (", percentage, "%) within 1σ"
}
...
task update_player(dt) {
    ...
    player_vel = rotate_point(player_vel, 2)  // 2° per frame
...
### Accuracy
- **Trigonometric functions:** ±0.0001 for angles in [-2π, 2π]
